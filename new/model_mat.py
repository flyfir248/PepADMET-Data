...
PATCH NOTE (target normalization): MATModel now carries y_mean/y_std as
buffers (so they're saved/restored with the state_dict automatically).
forward() is UNCHANGED -- it still returns a raw, normalized-scale scalar,
and training code should compute loss against (y - y_mean) / y_std. Use the
new predict() method wherever you want a real-scale LogP prediction (metrics
reporting, checkpoints for inference, Flask serving). This exists because
training with plain MSE directly against raw LogP values (~-10 to 0) at a
small fixed lr can be a slow, badly-scaled optimization target compared to
the sklearn baselines, which are scale-invariant. Old checkpoints trained
before this patch don't have these buffers; load with strict=False and
y_mean/y_std will default to (0.0, 1.0), i.e. a no-op, so old checkpoints
still load but won't have been trained with this benefit -- retrain to
actually get it.
"""
import math
