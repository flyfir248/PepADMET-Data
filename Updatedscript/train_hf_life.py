...
LABEL_CANDIDATES = ["label", "labels", "Label", "Labels", "half_life", "HalfLife"]
...
def load_train_val(variant_dir):
    """
    Load train + val for a variant, honestly held out for leaderboard metrics.
    If no val-*.parquet exists (your half_life folders only have train-*), carve
    an internal 80/20 split out of train instead of failing.
    """
    X_train_full, y_train_full, label_col = load_split(variant_dir, "train")
    try:
        X_val, y_val, _ = load_split(variant_dir, "val")
        X_train = X_train_full
        y_train = y_train_full
        print(f"  Found a separate val split on disk.")
    except FileNotFoundError:
        print(f"  No val-00000-of-00001.parquet found for {variant_dir} -- "
              f"carving an internal 80/20 split out of train for evaluation.")
        X_train, X_val, y_train, y_val = train_test_split(
            X_train_full, y_train_full, test_size=0.2, random_state=42
        )
    return X_train, y_train, X_val, y_val, label_col


def load_full_train_only(variant_dir, config, split):
    """
    Used only at export time to refit the winning model on ALL available real
    data. If there's a genuine val file, train+val gets concatenated normally.
    If there isn't one (half_life today), "val" returns an empty array so the
    concatenation in export_best_model is a no-op and we don't duplicate rows
    by re-adding an internally-split subset of train.
    """
    if split == "train":
        X, y, _ = load_split(variant_dir, "train")
        return X, y
    # split == "val"
    try:
        X, y, _ = load_split(variant_dir, "val")
        return X, y
    except FileNotFoundError:
        X_train, y_train, _ = load_split(variant_dir, "train")
        dim = X_train.shape[1]
...
def main():
    ...
    for variant_dir in VARIANTS:
        print(f"\n{'=' * 90}\nVariant: {variant_dir}\n{'=' * 90}")
        try:
            X_train, y_train, X_val, y_val, label_col = load_train_val(variant_dir)
    ...
    export_best_model(
        ...
        load_variant_fn=load_full_train_only,
    )
