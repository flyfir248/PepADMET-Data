"""
Trains hemolysis (classification: hemolytic vs non-hemolytic) across the
ChemBERTa and WT embedding variants, same pattern as your solubility /
permeability_penetrance scripts, and exports the winner to:
  models/hemolysis_best_model.joblib
  models/hemolysis_best_model_metadata.json
...
ASSUMPTION: hemolysis is treated as binary classification (hemolytic=1,
non-hemolytic=0). If your label column is continuous instead, this is the
wrong template -- use train_half_life.py's regression pattern instead.
"""
import warnings
...
ROOT = Path(".")
ENDPOINT_DIR = ROOT / "hemolysis"
RESULTS_DIR = ROOT / "results"
RESULTS_DIR.mkdir(exist_ok=True)

# Adjust these two folder names if yours differ slightly on disk.
VARIANTS = [
    "hemolysis__hemo_chemberta_with_embeddings",
    "hemolysis__hemo_wt_with_embeddings",
]
LABEL_CANDIDATES = ["label", "labels", "Label", "Labels"]
...
def main():
    ...
    out_path = RESULTS_DIR / "leaderboard_hemolysis.csv"
