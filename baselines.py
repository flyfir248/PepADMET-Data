...
def main():
    ...
    ap.add_argument("--full_grid", action="store_true",
                     help="use the paper-scale grid (8 RFR x 9 SVR candidates) instead of the fast default")
    args = ap.parse_args()

    rfr_grid = RFR_GRID_FULL if args.full_grid else RFR_GRID_FAST
    svr_grid = SVR_GRID_FULL if args.full_grid else SVR_GRID_FAST
