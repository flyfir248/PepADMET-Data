...
def main():
    ...
    ap.add_argument("--paper_faithful_arch_only", action="store_true",
                     help="DIAGNOSTIC: same architecture overrides as --paper_faithful (heads/layers/"
                          "kernel/lambdas/force field) but leaves --lr/--loss_reduction/--weight_decay/"
                          "--epochs/--patience at whatever you pass (default: our already-validated "
                          "lr=1e-4, mean-reduction, target-normalized recipe). Use this to isolate "
                          "whether a dead-gradient/non-learning run under --paper_faithful is caused by "
                          "the architecture itself (h=64/d_k=1, exp_elementwise kernel, lambda=0.1/0.6/0.3) "
                          "or by the lr=1e-3 + sum-reduction training recipe. Run this FIRST if "
                          "--paper_faithful fails to learn -- if this one trains normally, the problem is "
                          "isolated to lr/loss_reduction; if this one ALSO fails to learn, the problem is "
                          "in the architecture/kernel/lambda combination itself.")
    ...
    if args.paper_faithful:
        ...
        for k, v in overrides.items():
            ...
            setattr(args, k, v)
    elif args.paper_faithful_arch_only:
        overrides = dict(d_model=64, n_heads=64, n_layers=6, n_dense=2, leaky_slope=0.16,
                          lambdas="cpmp_pampa", distance_kernel_kind="exp_elementwise",
                          force_field="UFF", no_nb=True)
        print("[paper_faithful_arch_only] DIAGNOSTIC: architecture-only overrides "
              "(lr/loss_reduction/weight_decay/epochs/patience left as passed):")
        for k, v in overrides.items():
            print(f"    --{k} = {v}  (was {getattr(args, k)})")
            setattr(args, k, v)
        print(f"    --lr = {args.lr} (unchanged) --loss_reduction = {args.loss_reduction} (unchanged) "
              f"--weight_decay = {args.weight_decay} (unchanged)")
